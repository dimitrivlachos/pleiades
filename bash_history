...
bc_setup_atuin_daemon() {
  ...
  # Resolve the atuin binary. The curl installer puts it at ~/.atuin/bin/atuin
  # (the path baked into the units); package managers use /usr/bin/atuin. Prefer
  # the curl path so the units' default matches, else fall back to PATH.
  local default_bin="$HOME/.atuin/bin/atuin"
  local atuin_bin=""
  if [[ -x "$default_bin" ]]; then
    atuin_bin="$default_bin"
  elif atuin_bin="$(command -v atuin 2>/dev/null)"; then
    :
  else
    bc_log_error "atuin binary not found in ~/.atuin/bin or on PATH"
    bc_log_info "Install atuin, then re-run bc_setup_atuin_daemon"
    return 1
  fi
  ...
  # When atuin isn't at the units' default path, override ExecStart via a per-host
  # drop-in (generated, not symlinked) so the repo units stay untouched. Both the
  # daemon and its health check must point at the binary that exists here. When
  # atuin IS at the default path, clear any stale drop-in from a previous host.
  local dropin_svc="$dest_dir/atuin-daemon.service.d"
  local dropin_hc="$dest_dir/atuin-daemon-healthcheck.service.d"
  if [[ "$atuin_bin" != "$default_bin" ]]; then
    mkdir -p "$dropin_svc" "$dropin_hc"
    cat > "$dropin_svc/10-bash-config-binary.conf" <<EOF
# Generated by bc_setup_atuin_daemon — do not edit; re-run the function instead.
# Overrides the unit's default ~/.atuin/bin/atuin with the binary found on this host.
[Service]
ExecStart=
ExecStart=$atuin_bin daemon start
EOF
    cat > "$dropin_hc/10-bash-config-binary.conf" <<EOF
# Generated by bc_setup_atuin_daemon — do not edit; re-run the function instead.
[Service]
ExecStart=
ExecStart=/bin/sh -c 'timeout 5 $atuin_bin daemon status >/dev/null 2>&1 || { systemctl --user reset-failed atuin-daemon.service; systemctl --user restart atuin-daemon.service; }'
EOF
    bc_log_info "atuin at $atuin_bin — wrote ExecStart drop-in override"
  else
    rm -f "$dropin_svc/10-bash-config-binary.conf" "$dropin_hc/10-bash-config-binary.conf"
    rmdir "$dropin_svc" "$dropin_hc" 2>/dev/null || true
  fi
  ...
  # Clear a prior start-limit-hit (from a failed path) so enable --now can start.
  systemctl --user reset-failed atuin-daemon.service 2>/dev/null || true
